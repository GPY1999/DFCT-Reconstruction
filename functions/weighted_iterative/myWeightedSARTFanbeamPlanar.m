function [recon] = myWeightedSARTFanbeamPlanar(sino,sys_geo,varargin)
%MYWEIGHTEDSARTFANBEAMPLANAR Iterative recon: Simultaneous algebraic reconstruction techinique (SART), for fan-beam and planar detector
%   Weighted recon also supported
%   Adapated from myART. The difference is that all the rays at one angle are updated simulataneously.
...
%   2025/05/30 (GUO Peiyuan): first version
...
for nart = 1 : nART
    ...
    for iangle = 1 : length(beta_list)
        beta = beta_list(iangle) / 180 * pi; % source rotation angle, unit: radian
        if mod(iangle,100)==0 disp(strcat('angle:',32,num2str(beta_list(iangle)))); end
        
        recon_new = recon;
        for ipixel = 1 : sys_geo.pixel_num
            ...
            if ~isempty(point_weight)
                ...
                recon_new(point_index) = recon(point_index) - correct.*point_weight;
            end
        end
        recon = recon_new;
    end
