function [recon] = myWeightedIHelicalPlanar(sino,sys_geo,theta_list,varargin)
%MYWEIGHTEDIHELICALPLANAR Weighted inverted Radon transform, suitable for helical scan and planar detector
...
% set varargin
defaults = {'weight_function',@(x,y,theta)(1)};
...
for iangle = 1 : length(beta_list)
    ...
    I1_temp =  cos(theta) .* interp2((1:sys_geo.pixel_num3)',(1:sys_geo.pixel_num)',sino_conv(:,:,iangle),Wp,Vp,"linear",0) .* 1./U .* weight_map;
    I2_temp =  sin(theta) .* interp2((1:sys_geo.pixel_num3)',(1:sys_geo.pixel_num)',sino_conv(:,:,iangle),Wp,Vp,"linear",0) .* 1./U .* weight_map; 

    I1_temp(((sys_geo.z_start-z_mesh)>(beta+pi)*sys_geo.pitch/2/pi) | ((sys_geo.z_start-z_mesh)<(beta-pi)*sys_geo.pitch/2/pi)) = 0; % filter out the integration range of beta
    I2_temp(((sys_geo.z_start-z_mesh)>(beta+pi)*sys_geo.pitch/2/pi) | ((sys_geo.z_start-z_mesh)<(beta-pi)*sys_geo.pitch/2/pi)) = 0;
    I1 = I1 + I1_temp; I2 = I2 + I2_temp;
end
